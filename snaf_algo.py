...
class Actor(nn.Module):
    def __init__(self, state_shape, num_actions, name, checkpoints_dir="../Data/"):
        ...
        self.hidden2 = nn.Linear(in_features=512, out_features=256)
        self.hidden3 = nn.Linear(in_features=256, out_features=256)
        self.mean_layer3 = nn.Linear(in_features=256, out_features=num_actions)
        self.log_std_layer3 = nn.Linear(in_features=256, out_features=num_actions)
        self.P_log_std_layer3 = nn.Linear(in_features=256, out_features=num_actions)
    ...
    def forward(self, state):
        ...
        log_std = torch.clamp(self.log_std_layer3(x), -20, 2)
        std = log_std.exp()
        
        P_log_std = torch.clamp(self.P_log_std_layer3(x), -20, 2)
        P_std = P_log_std.exp()
        ...
        log_prob = log_prob.sum(1, keepdim=True).squeeze(-1)     

        return action, log_prob, torch.tanh(mean), P_std
...
class SNAFTrainer:
    def __init__(self, env, input_dims, alpha=0.001, beta=0.002, gamma=0.99, tau=0.05, 
        ...
        self.temperatue = 0.01 #self.log_alpha.exp().item()
        self.a_optimizer = optim.Adam([self.log_alpha], lr=self.alpha)
    ...
    def initialize_networks(self, n_actions, checkpoints_dir=None):
        """
        Initialize actor and critic networks for SNAF agent.
        """
        ...
        self.critic_1 = Critic(state_action_shape=self.input_dims,
                               name="critic_1", checkpoints_dir=checkpoints_dir).to(self.device)
        self.critic_2 = Critic(state_action_shape=self.input_dims,
        ...
        self.target_critic_1 = Critic(state_action_shape=self.input_dims, 
                                      name="target_critic_1", checkpoints_dir=checkpoints_dir).to(self.device)
        self.target_critic_2 = Critic(state_action_shape=self.input_dims, 
                                      name="target_critic_2", checkpoints_dir=checkpoints_dir).to(self.device)


        self.Qmodel_optimizer = optim.Adam(list(self.critic_1.parameters()) + list(self.critic_2.parameters()) + list(self.actor.parameters()), lr=self.beta)
        self.target_Qmodel_optimizer = optim.Adam(list(self.target_critic_1.parameters()) + list(self.target_critic_2.parameters()) + list(self.target_actor.parameters()), lr=self.beta)
    ...
    def optimize_model(self):
        """
        Function for agent learning that implements the SNAF algorithm.
        ...
        target_actions, nxt_logpi,pred_next_means , next_P_std= self.target_actor(next_states) #+ torch.clamp(torch.randn_like(actions) * 0.2, -0.5, 0.5)
        target_actions = torch.clamp(target_actions, self.min_action, self.max_action)

        with torch.no_grad():
            q1_new = self.target_critic_1(next_states).squeeze(1)
            q2_new = self.target_critic_2(next_states).squeeze(1)
            target = rewards + self.gamma* (1 - dones) * (torch.min(q1_new, q2_new) - (self.temperatue *nxt_logpi) )
            
            
        pred_actions, logpi, pred_means, P_std = self.actor(states)
        
        action_mu = (actions - pred_means).unsqueeze(2)
        nxt_P = torch.diag_embed(1 / P_std ** 2)
        A = - 0.5 * torch.bmm(torch.bmm(action_mu.transpose(2, 1), nxt_P), action_mu)[:, :, 0]
        
        q1 = self.critic_1(states).squeeze(1) + A.squeeze(-1)
        q2 = self.critic_2(states).squeeze(1) + A.squeeze(-1)
        ...
        # gradient descent
        self.Qmodel_optimizer.zero_grad()
        loss.backward()
        self.Qmodel_optimizer.step()
        ...
        alpha_loss = (-self.log_alpha.exp() * (logpi + self.target_entropy).detach()).mean()
    ...
    def snaf_train(self, n_episodes=1500, opt_steps=64, reward_weights=None, 
