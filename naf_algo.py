...
class NAFTrainer:
    ...
    def initialize_networks(self, n_actions, checkpoints_dir=None):
        """
        Initialize actor and critic networks for NAF agent.
        """
    ...
    def optimize_model(self):
        """
        Function for agent learning that implements the NAF algorithm.
        ...
        target_actions, next_P_std= self.target_actor(next_states) #+ torch.clamp(torch.randn_like(actions) * 0.2, -0.5, 0.5)
        ...
        with torch.no_grad():
            ...
            target = rewards + self.gamma* (1 - dones) * (torch.min(q1_new, q2_new))
            
            
        pred_means, P_std = self.actor(states)
    ...
    def naf_train(self, n_episodes=1500, opt_steps=64, reward_weights=None, 
