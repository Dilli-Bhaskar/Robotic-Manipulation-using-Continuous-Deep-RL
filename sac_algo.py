...
class SACTrainer:
    def __init__(self, env, input_dims, alpha=0.001, beta=0.002, gamma=0.99, tau=0.05, 
        ...
        self.temperatue = self.log_alpha.exp().item()
    ...
    def initialize_networks(self, n_actions, checkpoints_dir=None):
        """
        Initialize actor and critic networks for SAC agent.
        """
    ...
    def optimize_model(self):
        """
        Function for agent learning that implements the SAC algorithm.
        ...
        target_actions, nxt_logpi,_ = self.target_actor(next_states) #+ torch.clamp(torch.randn_like(actions) * 0.2, -0.5, 0.5)
        ...
        pred_actions, logpi, _ = self.actor(states)
        
        

        actor_loss = torch.mean((self.temperatue*logpi)-torch.min(self.critic_1(states, pred_actions), self.critic_2(states, pred_actions)) )
    ...
    def sac_train(self, n_episodes=1500, opt_steps=64, reward_weights=None, 
